...
const InventoryConfigModal: React.FC<InventoryConfigModalProps> = ({
  ...
  const loadCurrentConfig = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`process.env.REACT_APP_BACKEND_URL || process.env.REACT_APP_BACKEND_URL || "process.env.REACT_APP_BACKEND_URL || "process.env.REACT_APP_BACKEND_URL || process.env.REACT_APP_BACKEND_URL || process.env.REACT_APP_BACKEND_URL || "http://localhost:5000"""/api/inventory/config/${activeStore._id}`, {
        headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`process.env.REACT_APP_BACKEND_URL || process.env.REACT_APP_BACKEND_URL || "http://localhost:5000"/api/inventory/config/${activeStore._id}`, {
        method: 'POST',
        headers: {
