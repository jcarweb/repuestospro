...
async function testPromotionFix() {
  console.log('🧪 Probando corrección de promociones...\n');
  ...
  try {
    ...
    const testPromotion = {
      name: 'Promoción de Prueba - Corregida',
      description: 'Esta es una promoción de prueba para verificar la corrección',
      type: 'percentage',
    ...
    if (loadAfterData.promotions && Array.isArray(loadAfterData.promotions)) {
      const newPromotion = loadAfterData.promotions.find(p => p.name === 'Promoción de Prueba - Corregida');
      ...
      if (newPromotion) {
        console.log('Detalles de la nueva promoción:', {
          ...
          isMainStorePromotion: newPromotion.isMainStorePromotion
        });
      }
    }
    ...
    console.log(`✅ Carga después exitosa: ${loadAfterData.success ? 'SÍ' : 'NO'}`);
