...
async function assignSampleSubscriptions() {
  try {
    ...
    if (subscriptions.length === 0) {
      console.log('❌ No hay planes de suscripción disponibles. Ejecuta primero seed-subscriptions.js');
      return;
    }
    ...
    for (let i = 0; i < stores.length; i++) {
      ...
      updatedCount++;
      console.log(`✅ ${store.name} - ${subscriptions.find(s => s._id.toString() === subscriptionId.toString())?.name}`);
    }
