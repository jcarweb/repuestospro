...
async function testWithExactSecret() {
  try {
    console.log('🔑 Generando token con el JWT_SECRET exacto del middleware...');
    ...
    // 2. Generar token JWT con el mismo secreto que usa el middleware
    // El middleware usa: process.env.JWT_SECRET || 'fallback-secret'
    const JWT_SECRET = process.env.JWT_SECRET || 'fallback-secret';
    
    console.log(`\n🔍 JWT_SECRET detectado:`);
    console.log(`   - process.env.JWT_SECRET: ${process.env.JWT_SECRET || 'undefined'}`);
    console.log(`   - JWT_SECRET final: ${JWT_SECRET}`);
