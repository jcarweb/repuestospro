...
async function setupVehicleTypes() {
  try {
    ...
    console.log('\n🔍 Verificando tipos de vehículos requeridos...\n');
      ...
      try {
        ...
        if (vehicleType) {
          ...
          if (needsUpdate) {
            ...
            console.log(`✅ Tipo de vehículo '${vehicleTypeData.name}' actualizado`);
            updatedCount++;
          } else {
          ...
          await vehicleType.save();
          console.log(`✅ Tipo de vehículo '${vehicleTypeData.name}' creado exitosamente`);
          createdCount++;
        }
      } catch (error) {
    ...
    console.log(`\n📋 Tipos de vehículos finales (${finalTypes.length}):`);
