...
class InventoryController {
  ...
  async configureInventory(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      if (!inventoryType) {
        res.status(400).json({ success: false, message: 'El tipo de inventario es requerido' });
      }
      if (!distributionRules) {
        res.status(400).json({ success: false, message: 'Las reglas de distribución son requeridas' });
      }
        ...
        for (const childStoreId of config.childStores) {
          try {
            ...
            if (childConfig && childConfig.parentStore && childConfig.parentStore.toString() === storeId) {
              ...
              childConfig.parentStore = null as any;
              await childConfig.save();
            }
          } catch (error) {
  ...
  async getInventoryConfig(req: AuthenticatedRequest, res: Response) {
    try {
      const { storeId } = req.params;
      ...
      if (config && config.inventoryType === 'global' && config.parentStore) {
  ...
  async getUserInventoryConfigs(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const configsByStore = configs.reduce((acc, config) => {
        (acc as any)[config.store._id.toString()] = config;
        return acc;
      }, {});
  ...
  async updateStock(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const config = await InventoryConfig.findOne({ store: storeId });
      if (!config) {
        res.status(404).json({ success: false, message: 'Configuración de inventario no encontrada' });
      }
  ...
  async getStoreInventory(req: AuthenticatedRequest, res: Response) {
    try {
      const { storeId } = req.params;
  ...
  async distributeStock(req: AuthenticatedRequest, res: Response) {
    try {
      const { storeId } = req.params;
      ...
      const availableStock = mainInventory?.mainStock.available || 0;
      ...
      for (const childStore of childStores) {
        if (distributionAmount > 0) {
          const transfer = new InventoryTransfer({
            ...
            items: [{
              product: productId,
              productName: mainInventory?.product.toString() || 'Producto', // Esto debería ser el nombre real
  ...
  async getTransfers(req: AuthenticatedRequest, res: Response) {
    try {
      const { storeId } = req.params;
  ...
  async completeTransfer(req: AuthenticatedRequest, res: Response) {
    try {
      const { transferId } = req.params;
      const transfer = await InventoryTransfer.findById(transferId);
      if (!transfer) {
      ...
      for (const item of transfer?.items || []) {
        ...
        await ProductInventory.findOneAndUpdate(
          { product: item.product, store: transfer?.fromStore },
          {
            ...
            $set: { lastUpdated: new Date(), lastUpdatedBy: req.user?.id }
          }
        );
  ...
  async cleanDuplicateConfigs(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      ...
      configs.forEach(config => {
        ...
        if (!(configsByStore as any)[storeId]) {
          (configsByStore as any)[storeId] = [];
        }
        (configsByStore as any)[storeId].push(config);
      });
      ...
      for (const [storeId, storeConfigs] of Object.entries(configsByStore)) {
        if ((storeConfigs as any[]).length > 1) {
          ...
          const sortedConfigs = (storeConfigs as any[]).sort((a, b) =>
  ...
  async getAdminInventory(req: AuthenticatedRequest, res: Response) {
    try {
  ...
  async getAdminStats(req: AuthenticatedRequest, res: Response) {
    try {
  ...
  async exportAdminInventory(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { storeId, inventoryType, alert, search } = req.query;
  ...
  async importAdminInventory(req: AuthenticatedRequest, res: Response) {
    try {
      if (!req.file) {
        res.status(400).json({ success: false, message: 'No se proporcionó archivo CSV' });
      }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        try {
          ...
          errors.push(`Fila ${i + 1}: ${(error as Error).message}`);
          errorCount++;
        }
      }
      res.json({
  ...
  async getStoreManagerInventory(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      const { storeId, inventoryType, alert, search } = req.query;
      ...
      if (!user || !user.stores || user.stores.length === 0) {
        res.status(404).json({ success: false, message: 'No se encontraron tiendas asignadas' });
      }
  ...
  async getStoreManagerStats(req: AuthenticatedRequest, res: Response) {
    try {
      const { storeId } = req.params;
      const userId = req.user?.id;
      ...
      if (!userStoreIds.includes(storeId)) {
        res.status(403).json({ success: false, message: 'No tienes acceso a esta tienda' });
      }
  ...
  async getStoreManagerMovements(req: AuthenticatedRequest, res: Response) {
    try {
      const { storeId } = req.params;
      const userId = req.user?.id;
      // Verificar que el usuario tenga acceso a la tienda
  ...
  async exportStoreManagerInventory(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      const { storeId, inventoryType, alert, search } = req.query;
      // Obtener tiendas del usuario
  ...
  async importStoreManagerInventory(req: AuthenticatedRequest, res: Response) {
    try {
      if (!req.file) {
        res.status(400).json({ success: false, message: 'No se proporcionó archivo CSV' });
      }
  ...
  async getInventoryTransfers(req: AuthenticatedRequest, res: Response) {
    try {
