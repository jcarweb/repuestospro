...
export class AnalyticsController {
  ...
  private static async calculateProductsMetrics(products: any[], orders: any[]) {
    ...
    orders.forEach(order => {
      order.items?.forEach((item: any) => {
        if ((productSales as any)[item.productId]) {
          (productSales as any)[item.productId] += item.quantity;
        } else {
          (productSales as any)[item.productId] = item.quantity;
        }
      });
    });
    const totalSales = Object.values(productSales).reduce((sum: any, sales: any) => sum + sales, 0) as number;
  ...
  private static async calculateCustomersMetrics(orders: any[]) {
    ...
    orders.forEach(order => {
      if (order.customerId) {
        ...
        (customerOrderCounts as any)[customerId] = ((customerOrderCounts as any)[customerId] || 0) + 1;
      }
    });
  ...
  private static async calculateOrdersMetrics(orders: any[]) {
    ...
    const statusCounts = {};
    orders.forEach(order => {
      (statusCounts as any)[order.status] = ((statusCounts as any)[order.status] || 0) + 1;
    });
